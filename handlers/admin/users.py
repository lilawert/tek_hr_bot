...
from service.pincode import get_pincode_now
...
@router.message(Command("pincode"))
async def create_invite_link_cmd(message: Message):
    logger.debug('create_invite_link_cmd')

    pincode = await get_pincode_now()
    await message.answer(text=t('admin.users.link', pincode=pincode),
